...
# Usare lo starter STANDALONE (non NgModule) per coerenza con l'architettura del piano.
CI=true npx @ionic/cli@latest start judo-app-scaffold tabs --type=angular-standalone --capacitor --no-git --confirm
...
Verificare che lo scaffold sia standalone: `src/main.ts` deve usare `bootstrapApplication(...)`
e i componenti avere `standalone: true`. Se la versione di Ionic CLI non conosce
`angular-standalone`, usare `--type=angular` e confermare comunque che il risultato sia
standalone (Ionic 8 genera standalone di default); adattare i path dei file di routing
(`tabs.routes.ts` / `app.routes.ts`) a quanto effettivamente generato.
